...
function divSpecs(U, Fx, Fy, Fz, Fd_x, Fd_y, Fd_z, dt, J)
    ...
    for n = 1:Nspecs
        @inbounds dVdξ = c1 * (Fd_x[i,   j+2, k+2, n] - Fd_x[i+4, j+2, k+2, n]) +
                         c2 * (Fd_x[i+1, j+2, k+2, n] - Fd_x[i+3, j+2, k+2, n])
        @inbounds dVdη = c1 * (Fd_y[i+2, j,   k+2, n] - Fd_y[i+2, j+4, k+2, n]) +
                         c2 * (Fd_y[i+2, j+1, k+2, n] - Fd_y[i+2, j+3, k+2, n])
        @inbounds dVdζ = c1 * (Fd_z[i+2, j+2, k,   n] - Fd_z[i+2, j+2, k+4, n]) +
                         c2 * (Fd_z[i+2, j+2, k+1, n] - Fd_z[i+2, j+2, k+3, n])
