...
function time_step(rank, comm_cart, thermo, react)
    ...
    dsbuf_dx = unsafe_wrap(ROCArray, pointer(dsbuf_hx), size(dsbuf_hx))
    dsbuf_dy = unsafe_wrap(ROCArray, pointer(dsbuf_hy), size(dsbuf_hy))
    dsbuf_dz = unsafe_wrap(ROCArray, pointer(dsbuf_hz), size(dsbuf_hz))
    drbuf_dx = unsafe_wrap(ROCArray, pointer(drbuf_hx), size(drbuf_hx))
    drbuf_dy = unsafe_wrap(ROCArray, pointer(drbuf_hy), size(drbuf_hy))
    drbuf_dz = unsafe_wrap(ROCArray, pointer(drbuf_hz), size(drbuf_hz))
    ...
    for tt = 1:ceil(Int, Time/dt)
        ...
        for KRK = 1:3
            ...
            if KRK == 2
                ...
                @roc groupsize=nthreads gridsize=ngroups linComb(ρi, ρn, Nspecs, 0.25f0, 0.75f0)
            elseif KRK == 3
                @roc groupsize=nthreads gridsize=ngroups linComb(U, Un, Ncons, 2/3f0, 1/3f0)
                @roc groupsize=nthreads gridsize=ngroups linComb(ρi, ρn, Nspecs, 2/3f0, 1/3f0)
            end
