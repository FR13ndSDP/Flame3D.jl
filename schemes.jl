...
function WENO_x(F, ϕ, S, Fp, Fm, NV)
    ...
    elseif ϕx < hybrid_ϕ2
        ...
        for n = 1:NV
            ...
            @inbounds F[i-NG, j-NG, k-NG, n] = (fp + fm) * tmp
        end
    else
        for n = 1:NV
            @inbounds fp = Fp[i-1, j, k, n] + 0.5f0*minmod(Fp[i, j, k, n] - Fp[i-1, j, k, n], 
...
function weno7_x(F, S, Fp, Fm, NV)
    ...
    for n = 1:NV
        ...
        # alpha
        α1 = 1/(WENOϵ+Is1*ss)^2
        α2 = 12/(WENOϵ+Is2*ss)^2
        α3 = 18/(WENOϵ+Is3*ss)^2
        α4 = 4/(WENOϵ+Is4*ss)^2
